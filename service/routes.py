...
def get_products(product_id):
    """Gets a product from passed id"""
    app.logger.info(f"Request to Read a Product with id: {product_id}")

    product = Product.find(product_id)
    if not product:
        abort(status.HTTP_404_NOT_FOUND, description=f"Product not found - invalid id {product_id}")
...
def update_products(product_id):
    """Updates a product using its passed id"""
...
def delete_products(product_id):
    """Deletes a product from its passed id"""
